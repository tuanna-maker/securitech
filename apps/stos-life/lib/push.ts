...
/** Register device token for push (stub until FCM/APNs configured on Expo) */
export async function registerPushToken(userId: string, tenantId?: string) {
  let tid = tenantId;
  if (!tid) {
    const { data } = await db.from("residents").select("tenant_id").eq("user_id", userId).maybeSingle();
    tid = data?.tenant_id;
  }
  if (!tid) return null;
