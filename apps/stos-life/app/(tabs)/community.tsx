...
export default function CommunityScreen() {
  ...
  return (
    <Screen title="" largeTitle={false} scroll>
      <LifeHeader mode="centered" title="Cộng đồng" subtitle="Thông báo từ ban quản lý" notifyCount={3} />
      ...
      {!isLoading && !isError && rows.length === 0 ? (
        <EmptyState
          ...
          subtitle="Tin từ ban quản lý sẽ hiện tại đây."
          onAction={() => trackEvent("empty_state_view", { screen: "community" })}
        />
      ) : null}
      {!isLoading && !isError && rows.length > 0 ? (
        <GroupedSection>
